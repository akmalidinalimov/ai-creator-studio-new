// Web mirror of supabase/functions/telegram-bot-webhook/homework-stats.ts.
// Kept in sync manually — both files implement the same effective-grade rule.
...
export function effectiveLeafGrades(leaves: LeafLite[], subs: SubmissionLite[]): LeafEffective[] {
...
export function summarizeHomework(rows: LeafEffective[]): HomeworkSummary {
  ...
  let submittedCount = 0, scoredCount = 0, earned = 0, maxTotal = 0, maxScored = 0;
  for (const r of rows) {
...
export function leavesFromAssignments<T extends { id: string; parent_id: string | null; is_active?: boolean | null }>(all: T[]): T[] {
  const active = all.filter((a) => a.is_active !== false);
  const parentIdsWithSap = new Set(active.filter((a) => a.parent_id).map((a) => a.parent_id as string));
