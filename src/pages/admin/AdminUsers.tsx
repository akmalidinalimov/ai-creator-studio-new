...
import { Plus, Upload as UploadIcon, Search, Copy, RefreshCw, Trash2 } from "lucide-react";
...
interface CsvRow { name: string; email: string; password?: string; telegram_username?: string; role: "student" | "admin"; valid: boolean; reason?: string }
...
export default function AdminUsers() {
  ...
  const handleAdd = async () => {
    ...
    if (r?.status === "created") {
      ...
      setOpenAdd(false); setNewName(""); setNewEmail(""); setNewPassword(randPassword()); setNewTg(""); setNewRole("student"); setNewCourses(new Set());
      reload();
    } else {
  ...
  const parseCsv = (txt: string) => {
    ...
    const rows: CsvRow[] = lines.map((line, i) => {
      if (i === 0 && /name.*email/i.test(line)) return null as any;
      const parts = line.split(",").map((p) => p.trim());
      const [name, email, password, tg, role] = parts;
      ...
      let valid = true;
      if (!validEmail) { valid = false; reason = "Invalid email"; }
    ...
    <PageShell>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!loading && filtered.length === 0 && <tr><td colSpan={9} className="p-6 text-center text-muted-foreground">No users found</td></tr>}
                {filtered.map((u) => (
      ...
      <Dialog open={openCsv} onOpenChange={setOpenCsv}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">Format: <code>name,email,password,telegram_username,role</code> — password, telegram, and role are optional. Defaults: random password, no telegram, role=student.</p>
            <Textarea rows={6} value={csvText} onChange={(e) => { setCsvText(e.target.value); parseCsv(e.target.value); }} placeholder="Tg User One,tg1@test.io,tg1pass,@tguser1,student&#10;Admin Two,admin2@test.io,a2pass,,admin" />
            {csvParsed.length > 0 && (
              <div className="border rounded-md max-h-64 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted/40 text-xs sticky top-0"><tr><th className="text-left p-2">Name</th><th className="text-left p-2">Email</th><th className="text-left p-2">Telegram</th><th className="text-left p-2">Role</th><th className="text-left p-2">Status</th></tr></thead>
                  <tbody>
                    {csvParsed.map((r, i) => (
      ...
      <Sheet open={!!manageUser} onOpenChange={(o) => !o && setManageUser(null)}>
        <SheetContent className="w-full sm:max-w-md overflow-y-auto">
          {manageUser && (
            <>
              <SheetHeader><SheetTitle>{manageUser.name || manageUser.email}</SheetTitle></SheetHeader>
