...
-- where <OLD ref> is wpdztrijasgmxgliwddr — the project's ORIGINAL Supabase ref, not production
-- (cdyidatkegxwhtuoqxly). The production schema was built on 2026-07-05 by replaying the old
-- project's migrations, and the replay carried forward this function's last definition
-- (20260615020000_tier_enforcement.sql) with the old URL baked in. It was broken from the first day
-- in production; no later migration copied it forward.
--
-- WHAT IT DID, measured (read-only, 2026-09-26):
--   * Production's notify-completion has not run once for a lesson since 2026-07-05. notifications_log
--     has zero lesson_complete / module_complete / course_complete rows after 2026-07-05 14:36 UTC.
--     5,239 first-time lesson completions since then produced no completion message.
--   * The old project is ALIVE (it is the original Lovable-hosted environment). Its gateway answered,
--     and its own notify-completion returned 403 {"error":"forbidden"} — its secret does not match
--     ours — until 2026-09-25 17:19 UTC, then 5-second timeouts. So it most likely messaged no one.
--   * But every one of those calls carried PRODUCTION's internal_fn_secret() in the
--     x-internal-secret header, plus the student's user_id and lesson_id — roughly 754 calls in the
--     last 30 days, an estimated ~4,000 since the cutover. That secret has therefore been sent, on
--     every completion, to infrastructure this project does not control. It should be ROTATED; that
--     is an owner action and is not done here.
--
-- WHY THIS REMOVES THE DISPATCH INSTEAD OF RE-POINTING IT AT PRODUCTION. Pointing the URL at the right
-- project would wake a student-facing path that has been silent for 12 weeks, and it would DOUBLE-SEND:
--   * Module completion is already celebrated twice over by paths that took over while this was dead:
--     trg_enqueue_module_complete → nudge_module_celebrations → detect-and-nudge (133 sends in the last
--     30 days), and evaluate_lesson_badges → module_complete badge → notify-badge-award. Re-enabling
--     notify-completion would add its own message plus a photo: 2–4 messages for one module.
--   * Course completion is already sent as the course_complete badge (16 in 30 days).
--   * notify-completion also has defects of its own: it declares "module complete" on the last lesson
--     BY POSITION even when earlier lessons are unfinished, it ignores quiet hours, the nudge opt-out
--     and archived status (detect-and-nudge respects all three), and it has no idempotency beyond an
--     unlocked read in this function.
-- The only thing notify-completion did that nothing else does is a per-LESSON "completed, next: X"
-- message — which students have not received for 12 weeks. So removing the call changes NOTHING a
-- student currently experiences, stops the secret leaving the building, and avoids reviving a buggy
-- notifier. If per-lesson messages are wanted again, they deserve a proper rebuild (a trigger on the
-- FIRST completion so web completions count too, a unique-per-(user,lesson) queue, a sendTelegram
-- drainer that respects quiet hours and opt-out, off by default behind a platform_settings flag) —
-- that is a product decision, deliberately not made here.
--
-- WHAT CHANGED IN THE FUNCTION, and nothing else:
...
--     condition, so it was now a dead read of lesson_progress on every call of a function called
--     ~1,031,693 times. Removing it cannot change any result.
...
-- is_module_tier_locked tier gate (whose non-enrolled pass-through is deliberate here and documented
-- in its own comment), the 10-second delta clamp, the duration logic, the 85% / last-20-seconds
-- completion rule, the Asia/Tashkent daily_watch_summary date, and the RETURN shape the client reads.
--
-- NOTHING TO BACKFILL, deliberately. Re-sending 5,239 "you completed a lesson" messages up to twelve
-- weeks late would be worse than the gap, and the module and course celebrations were delivered by
-- the other paths throughout.
--
...
-- self-describing and so the author-time lint (scripts/check-migration-grants.mjs) sees the revoke.
--
-- Idempotent + replay-safe: CREATE OR REPLACE and declarative GRANT/REVOKE. The self-test is
-- read-only and never calls this function, because it WRITES (lesson_progress, daily_watch_summary).
...
BEGIN
  ...
  -- 20260926160000 for why this was removed rather than re-pointed.
...
begin
  ...
  -- 1. The dispatch, the foreign URL, the foreign JWT and the secret read are all gone.
  --    (The function body above deliberately contains no comment mentioning any of these strings, so
  --    these negative matches cannot be fooled by explanatory text — a trap hit earlier today.)
  ...
  -- 2. Nothing else was lost. Truncating or mangling a hot-path function is the real risk of a
  --    rewrite, so assert each piece of logic that must survive is still there.
  if _src !~ 'auth\.uid\(\)'                          then _bad := _bad || 'lost the auth.uid() check; '; end if;
  if _src !~ 'is_module_tier_locked'                  then _bad := _bad || 'lost the tier gate; '; end if;
  if _src !~ 'LEAST\(GREATEST\(COALESCE\(p_delta_seconds' then _bad := _bad || 'lost the 10s delta clamp; '; end if;
  if _src !~ 'ON CONFLICT \(user_id, lesson_id\)'     then _bad := _bad || 'lost the lesson_progress upsert; '; end if;
  if _src !~ '0\.85'                                  then _bad := _bad || 'lost the 85% completion rule; '; end if;
  if _src !~ 'new_dur - 20'                           then _bad := _bad || 'lost the last-20-seconds rule; '; end if;
  if _src !~ 'Asia/Tashkent'                          then _bad := _bad || 'lost the Tashkent watch date; '; end if;
  if _src !~ 'daily_watch_summary'                    then _bad := _bad || 'lost daily_watch_summary; '; end if;
  ...
  values (null, 'track_video_progress_stale_dispatch_removed',
          jsonb_build_object(
