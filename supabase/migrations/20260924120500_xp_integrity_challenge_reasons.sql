-- XP integrity report: account for the three challenge reasons.
--
-- WHY: xp_award_integrity_watchdog() reconciles every XP reason it knows against its source table and
-- lists the ones it deliberately cannot verify. Challenge Phase 1 (20260920103000) added three new
-- reasons - challenge_group_media, challenge_question, challenge_instagram - and the report has been
-- silent about them ever since. Silence in an integrity report is the one thing it must never do: a
-- reader cannot tell "verified and fine" from "never looked at".
--
-- These three cannot be verified from a source table the way lesson_complete is. They are derived
-- from webhook_inbox.raw_update, which is raw Telegram traffic and not a normalized record of awards;
-- and the Instagram award is attributed by a Meta webhook payload that lives outside this database
-- entirely. So they are declared unverifiable-by-design and listed, rather than silently dropped or
-- checked against a source that cannot answer.
--
-- STILL COVERED, and unchanged: aggregate_drift (user_xp vs sum(xp_events)) spans EVERY reason
-- including these, so a challenge point that corrupts a student's total is still caught. The
-- amount_mismatch list is an explicit `reason in (...)` and does NOT include the challenge reasons,
-- because their point values are owner-tunable in platform_settings and a fixed expected amount would
-- alarm the first time the owner changes one.
--
-- The function body is reproduced from 20260823130200 with that single array literal extended, because
-- CREATE OR REPLACE FUNCTION needs the whole body. The trailing REVOKE and the GRANT to service_role
-- are both reproduced too: CREATE OR REPLACE preserves existing privileges, but restating them keeps
-- this file's end state explicit instead of dependent on what a previous migration happened to leave
-- behind. Deliberately NOT reproduced: the original file's deploy-time self-test, which would run the
-- full watchdog (and could DM admins) on every replay of this migration.
...
begin
  ...
  _report := jsonb_build_object(
    ...
    'unverifiable_by_design', jsonb_build_array('streak_7','streak_30','teacher_answer','teacher_queue_clear',
      'teacher_grade_ontime','teacher_active_day','community_help','community_question',
      'challenge_group_media','challenge_question','challenge_instagram'),
