-- Make start_homework_resubmission idempotent: if the row is already in resubmission state
-- (score_is_stale = true and score IS NULL), do nothing destructive.
...
BEGIN
  ...
  -- Idempotent: already in resubmission state — no-op.
  IF v_row.score_is_stale = true AND v_row.score IS NULL THEN
    RETURN v_row;
  END IF;
...
-- Add stats_dirty_at to profiles for downstream stat refresh signals.
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS stats_dirty_at timestamptz;