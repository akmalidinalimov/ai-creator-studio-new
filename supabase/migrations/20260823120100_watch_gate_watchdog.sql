-- Detector for the watch-gate class (incident doctrine step 5): fire BEFORE the next complaint.
-- The forward-fix (PR #105) should make new false completions ~0; a spike means the completion gate
-- (src/lib/watchGate.ts) regressed or a new bypass appeared. Modeled on enrollment_watchdog /
-- grade_orphan_watchdog (app_settings state + daily-throttled admin Telegram DM + admin_actions log).
...
declare
  ...
  -- Tunable: how many <20%-watched "completions" in 24h count as an anomaly. Set above the
  -- transition trickle (students on a stale cached build still using the old gate until they reload).
  _threshold int := 8;
begin
  -- NEW false completions in the last 24h: a real Bunny video, completed, but watched < 20% of a
  -- KNOWN length (>= 90s) — the exact signature the fixed gate should now prevent.
  ...
  if _should_alert or _recovered then
    ...
    -- DB-visible health signal (incident doctrine): durable, queryable trail beyond the DM.
    begin
...
-- House convention: SECURITY DEFINER watchdogs are service_role / cron only, never PostgREST-exposed.
...
-- Daily at 06:15 UTC (11:15 Tashkent). Idempotent (deploy-concurrency-safe); a scheduling failure is
-- surfaced durably in admin_actions rather than silently unscheduled (teacher-engagement precedent).
do $$
begin
