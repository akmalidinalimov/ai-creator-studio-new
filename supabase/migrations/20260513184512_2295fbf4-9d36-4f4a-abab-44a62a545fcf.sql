...
BEGIN
  ...
  IF v_row.score IS NULL THEN
    -- No prior grade to preserve; still bump attempt + mark stale=false.
  ...
  -- Preserve the prior grade/comment so the student keeps seeing it.
  -- Mark it as stale; teacher's grading flow will overwrite + flip back to false.
  UPDATE public.homework_submissions
     SET attempt_number = COALESCE(attempt_number, 1) + 1,
