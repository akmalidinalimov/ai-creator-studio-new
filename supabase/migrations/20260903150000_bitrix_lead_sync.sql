...
-- 1) Per-lead sync state (all nullable / default so existing rows and the submit-lead upsert are unaffected).
...
-- 2) Retro-mark: do NOT back-dump every historical lead into Bitrix when the webhook is first set. Mark all
--    leads that exist now as synced so the drainer only ever forwards NEW leads. One-shot guarded so the
--    "a migration's SQL can run >1x" quirk can't later re-mark a genuinely-unsynced new lead as done.
do $$
begin
  if not exists (select 1 from public.app_settings where key = 'bitrix_leads_retromark_done') then
    update public.leads set bitrix_synced = true where bitrix_synced = false;
    insert into public.app_settings (key, value)
      values ('bitrix_leads_retromark_done', jsonb_build_object('at', now()))
...
begin
  ...
  if _should_alert or _recovered or _b_should or _b_recovered then
    ...
    if _tok is not null and _tok <> '' then
      ...
      -- second (bitrix) message, sent as its own line/alert
      if _b_should then
