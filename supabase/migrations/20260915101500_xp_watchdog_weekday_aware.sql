-- XP throughput watchdog: compare like with like, say so when it cannot, and stop diagnosing.
--
...
--      watching. That state is now detected, held non-breaching on purpose, and recorded as a
--      DB-visible `xp_watchdog_low_history` row (edge-triggered, so it cannot spam).
--   4. The message reports what was measured and what it was measured against, and stops guessing at
--      the cause.
--
...
begin
  ...
  insert into public.app_settings (key, value) values ('xp_throughput_watchdog_state', jsonb_build_object(
    ...
    'weeks_with_data', _weeks, 'low_history', _low_history,
  ...
  return jsonb_build_object('today', _today, 'baseline', round(_baseline, 1),
    'weeks_with_data', _weeks, 'low_history', _low_history, 'breach_streak', _streak,
...
-- A genuine ongoing collapse is not hidden by this: the next two cron runs re-confirm and alert
-- within ~12h.
insert into public.app_settings (key, value)
values ('xp_throughput_watchdog_state', jsonb_build_object(
  'alerting', false, 'breach_streak', 0, 'last_alert_ms', 0,
  'note', 'reset by 20260915101500 -- alerting now means breached AND confirmed'))
