...
//
// Requires a valid user JWT. Phase 0 (M19): the admin-only token-variant "verify"
// oracle and the "debug" response — which reflected secret length, first/last
// characters, hex byte count, and the raw hash-input prefix — have been REMOVED.
// This function now only signs; it never returns any secret-derived metadata.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const token = await sha256Base64Url(`${KEY}${tokenPath}${expires}`);
